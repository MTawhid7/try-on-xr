...
// Fix: Accept 'THREE.Mesh | null' to match useRef initialization
export function useGarmentInteraction(meshRef: React.RefObject<THREE.Mesh | null>) {
    const { camera, raycaster, gl } = useThree();
    const { grabParticle, moveParticle, releaseParticle, isRunning } = useSimulationStore();
    ...
    useEffect(() => {
        ...
        const handleDown = (e: PointerEvent) => {
            ...
            // Fix: Explicitly create Vector2
            raycaster.setFromCamera(new THREE.Vector2(x, y), camera);

            // meshRef.current is checked above, so it's safe
    ...
    }, [camera, raycaster, gl, isRunning, grabParticle, moveParticle, releaseParticle, meshRef]);
}